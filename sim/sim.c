...
/* modififed to check for a zero length file and print an intuitive 
 * mesage because the normal endfile message confuses the students
 * -- kdc, December 2023
 */
...
int main(int argc, char* argv[]) {
  ...
  set_stall_mode(3);  /* default for Lab 1 is stall mode 1 */
  ...
  while(current_argument < argc) {
	...
	else { /* check for an empty file and print useful message */
	...
	if (*c == 'r') {
	  ...
	  temp  = IntFromString(argv[current_argument+1],&legal);
	  if (!legal) {
	    fprintf(stderr,"\nError: argument to -s ('%s') is not a valid number.\n",
		    argv[current_argument+1]);
	    fprintf(stderr,"execution halts.\n\n");
	    die_quickly(-1);
	  }
...
void print_help()
{
    ...
    printf("    -v                 print the simulator's version number\n\n");
